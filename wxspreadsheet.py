...
class ContextMenu(wx.Menu):
    ...
    def _getSelectionRowSize(self):
        ...
        if self.parent.GetSelectionBlockTopLeft():
            numrows = (
                self.parent.GetSelectionBlockBottomRight()[0][0]
                - self.parent.GetSelectionBlockTopLeft()[0][0]
                + 1
            )
        else:
    ...
    def _getSelectionColSize(self):
        ...
        if self.parent.GetSelectionBlockTopLeft():
            numcols = (
                self.parent.GetSelectionBlockBottomRight()[0][1]
                - self.parent.GetSelectionBlockTopLeft()[0][1]
                + 1
            )
        else:
...
class SpreadsheetTextCellEditor(wx.TextCtrl):
    ...
    def __init__(self, parent, id, grid):
        wx.TextCtrl.__init__(
            self, parent, id, "", style=wx.NO_BORDER | wx.TE_PROCESS_ENTER
        )
...
class Spreadsheet(wx.lib.sheet.CSheet):
    ...
    def ReadCSV(self, csvfile, _delimiter=",", _quotechar="#"):
        ...
        with open(csvfile, "rU") as inputfile:
            csv_reader = csv.reader(
                inputfile, delimiter=_delimiter, quotechar=_quotechar
            )
            try:
    ...
    def WriteCSV(
        self, csvfile, _delimiter=",", _quotechar="#", _quoting=csv.QUOTE_MINIMAL
    ):
