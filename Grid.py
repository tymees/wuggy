...
class Grid(wxspreadsheet.Spreadsheet):
    ...
    def ImportData(self):
        dialog = wx.FileDialog(self.GetParent(), "Choose a file", "", style=wx.FD_OPEN)
    ...
    def SaveData(self, headers=False):
        dialog = wx.FileDialog(self, "Choose a file", "", style=wx.FD_SAVE)
        if wx.ID_OK == dialog.ShowModal():
            path = dialog.GetPath()
...
class InputGrid(Grid):
    ...
    def Segment(self, generator, replace=False):
        ...
        for rownum, (word, segments, expression) in enumerate(array):
            ...
            if segments == "" or replace == True:
                segments = generator.lookup(word)
                segments = "" if segments == None else segments
                self.SetCellValue(rownum, 1, segments)
                if word != "" and segments == "":
                    message = "row %d: no segments found for %s" % (rownum + 1, word)
